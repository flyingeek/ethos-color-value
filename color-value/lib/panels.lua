...
local function fillLogicPanel(panel, widget, grabFocus)
        ...
        if widget.source.decimals and widget.source:decimals() then
            ...
            while k < 2 and j >= 0 do
                ...
                if j ~= precision then
                    table.insert(buttons,
                        {
                            label = string.format("%." .. tostring(j) .. "f", widget.source:value() or 0, widget.source),
                            action = function()
                                appendTag(i, tag, method)
                                return true
                            end
                        }
                    )
    ...
    if lcdWidth <= 480 then
        colorWidth = 50
        choiceWidth = 60
        maxDialogChars = 50
    elseif lcdWidth <= 640 then
        colorWidth = 64
        choiceWidth = 80
        maxDialogChars = 50
    end
