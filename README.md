# Color Value : Lua Ethos Widget [[download]](https://github.com/flyingeek/ethos-color-value/releases/latest)

![Release](https://img.shields.io/endpoint?url=https://flyingeek.github.io/ethos-color-value/release.json) ![Preview](https://img.shields.io/endpoint?url=https://flyingeek.github.io/ethos-color-value/preview.json)

## Features

- Mimics the Ethos's Value widget
- Adapts to many widget sizes
- Dark and Light themes supported
- Up to 5 configurable thresholds, each with his own color
- Telemetry values may display min/max inside the widget or using the contextual menu
- Works fine with timers too
- Background color support
- Custom states (including special tags)
- Beautiful [documentation](https://flyingeek.github.io/astro-color-value/) including a widget simulator
- International language support (cs/de/en/es/fr/it)

Note : Ethos ≥ 1.6.3 required

## ScreenShots

![home screen](screenshots/home_page.webp "3 widgets on Home Page")
<p align="center">A view of the home page</p>

![config RxBatt](screenshots/config_panel_rxBatt.webp "RxBatt configuration screen")
<p align="center">RxBatt configuration screen (Min/Max OFF)</p>

![menu RxBatt](screenshots/menu_rxBatt.webp "RxBatt contextual menu")
<p align="center">RxBatt contextual menu</p>

![config Timer1](screenshots/config_panel_timer1.webp "Timer1 configuration screen")
<p align="center">Timer colorization</p>

![config VFT](screenshots/config_panel_vfr.webp "VFR configuration screen")
<p align="center">VFR configuration screen (Min/Max ON)</p>

![going grazy](screenshots/go-crazy-demo.webp "A crazy demo")
<p align="center">Demo screen</p>

## Do not miss the documentation

You can really try the widget in your browser with nice guided examples --> [Documentation](https://flyingeek.github.io/astro-color-value/)

## What's in the box ?

After installation, two new widgets become available: **Color Value** and **Color Telemetry**. They use the same underlying code, but **Color Telemetry** filters the source to a telemetry sensor—so if you’re using telemetry, it’s faster to set up with Color Telemetry.

To install or download, go to the [latest release page](https://github.com/flyingeek/ethos-color-value/releases/latest) and follow the instructions.
